...
<script>	
	...
	function check(my)
	{
		...
		{
			var con=confirm("방 이름을 그대로 사용하시겠습니까?");
		}
		else
			return true;
	}
...
<h2>방 추가</h2>
<hr>
<form name="pkc" method="post" action="room_update_ok.jsp" onsubmit="return check(this)">
<input type="hidden" name="id" value="${rdto.id }">
<table align="center">
	<tr>	
		<td>방 이름</td>
		<td>
			<input type="text" name="bang" value="${rdto.bang }" onblur="bang_check(this.value)" >
			<span id="text"></span>
		</td>
	</tr>
	<tr>	
		<td>최소인원</td>
		<td><input type="text" name="min" value="${rdto.min }"></td>
	</tr>
	<tr>	
		<td>최대인원</td>
		<td><input type="text" name="max" value="${rdto.max }"></td>
	</tr>
	<tr>	
		<td>1박 가격</td>
		...
		<tr>	
		<td>방 구조</td>
		<td><input type="text" name="type" value="${rdto.type}"></td>
	</tr>
	<tr>	
		<td>내용</td>
